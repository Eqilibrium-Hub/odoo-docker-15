...
ARG ODOO_USER=odoo
ARG ODOO_USER_UID=123
ARG ODOO_USER_GID=128
...
RUN set -x; \
        ...
        && su - ${ODOO_USER} -s /bin/bash -c "git clone https://github.com/waqas36/midrasolutions_module.git --depth 1 --branch master --single-branch ." \
        && mkdir /var/log/odoo \
